...
const ListObjects = () => {
    ...
    const listObjectsOfBucket = async () => {
        try {
            const objectsStream = mc.extensions.listObjectsV2WithMetadata('youtube', '', true, '')
            objectsStream.on('data', async (chunk) => {
                const {name: objectName} = chunk
                const presignedUrl = await mc.presignedGetObject('youtube', objectName, 24 * 60 * 60)
                setObjects((prev) => {
